 ...
 * 
 * To apply substitution to agents 
 * 
		  ...
		  List.fold_left 
		    (fun prefix_set (oldsite,image)  -> 
			 ...
			 List.fold_left 
		       ...
		       in 
			 extended_prefix_set)
		    [[name,agent_name],[],SiteSet.empty] 
		    agent.interface  in 
		  List.fold_left  
		    (fun sol (annotation,int,_) -> (List.rev annotation,case.lineage,{agent_name=agent_name;interface=List.rev int})::sol) 
		    sol interface_set )
	     [] cases 
       end
     with Not_found -> 
